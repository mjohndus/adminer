...
";
	...
	foreach ((array) $databases as $db) {
		...
		if ($connection->selectDatabase($db)) {
			if ($is_sql && preg_match('~CREATE~', $style) && ($create = $connection->getResult("SHOW CREATE DATABASE " . idf_escape($db), 1))) {
				set_utf8mb4($create);
			...
			if ($is_sql) {
				...
				if ($_POST["events"]) {
					foreach (get_rows("SHOW EVENTS", null, "-- ") as $row) {
						$create = remove_definer($connection->getResult("SHOW CREATE EVENT " . idf_escape($row["Name"]), 3));
						set_utf8mb4($create);
